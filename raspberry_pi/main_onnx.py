...
MODEL_PATH = "/home/pi/Downloads/best_320.onnx"
...
# ═══════════════════════════════════════════════════════
#  LOAD ONNX MODEL  (fixed — task=detect)
# ═══════════════════════════════════════════════════════
...
def load_model():
    ...
    if not model_path.exists():
        ...
        print(f"  Export from Colab: model.export(format='onnx', imgsz={INPUT_SIZE})")
        exit(1)
    print(f"[MODEL] Loading ONNX model: {MODEL_PATH}")
    model = YOLO(str(model_path), task='detect')  # task fix — correct tensor parsing
...
# ═══════════════════════════════════════════════════════
#  PARSE DETECTIONS  (fixed for ONNX coordinate parsing)
# ═══════════════════════════════════════════════════════

def parse_detections(results, frame_w, frame_h):
    detections = []
    ...
    for r in results:
        ...
        for box in r.boxes:
            ...
            # ── Detect normalised coords (ONNX outputs 0-1 range) ─────────
            # .pt  → pixel coords  e.g. [153.0, 186.0, 810.0, 715.0]
            # .onnx → normalised   e.g. [0.23,  0.0,   1.74,  0.0  ]
            if max(x1, y1, x2, y2) <= 2.0:
                # normalised → convert to pixels
                x1 = int(x1 * frame_w)
                y1 = int(y1 * frame_h)
                x2 = int(x2 * frame_w)
                y2 = int(y2 * frame_h)
...
def draw_detections(frame, detections, fps, infer_ms, frame_no):
    ...
    overlay = [
        ...
        f"Model: best.onnx  |  conf={CONF_THRESH}",
    ]
...
def main():
    ...
    print("[WINDOW] ✅ Live window open — press q to quit\n")
    ...
    try:
        while True:
            ...
            # pass frame dimensions for correct ONNX coordinate parsing
            detections = parse_detections(results, actual_w, actual_h)
